...
def balanced(m):
    ...
    if is_weight(m):
        ...
        left_side = length(left(m)) * total_weight(left_end)
        right_side = length(right(m)) * total_weight(right_end)
        return left_side == right_side and balanced(left_end) and balanced(right_end)
def totals_tree(m):
