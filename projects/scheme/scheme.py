...
def do_define_macro(expressions, env):
    ...
    check_form(expressions, 2)
    target = expressions.first
    check_formals(target)  ## this helps to check if it is a symbol
    check_form(target, 0)  ## this helps to check if it is Pair
    name_macro, formals = target.first, target.rest
    body = expressions.rest
    env.define(name_macro, MacroProcedure(formals, body, env))
    return macro_name
    # END Problem 20
