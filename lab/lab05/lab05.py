...
def distance(city1, city2):
    ...
    return float(sqrt(((lat2-lat1)**2)+((lon2-lon1)**2)))
...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    start = make_city('starting', lat, lon)
    distance1 = distance(start, city1)
    distance2 = distance(start, city2)
...
def sprout_leaves(t, vals):
    ...
    t = tree(label(t), vals)
    new_tree = tree(label(t), [tree(v) for v in vals])
    if is_leaf(new_tree):
        return tree(label(new_tree).append())"""
    if is_leaf(t):
...
#if i call sprout leaves on every branch ill ge the right numbers
